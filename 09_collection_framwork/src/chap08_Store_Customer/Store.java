...
/**
  ...
  public Map<String, Object> sell(double money, int count) {
	  // 구매를 원하는 상품의 개수가 상점이 보유한 상품의 개수보다 많은 경우
    if(this.count < count) {
    	System.out.println("판매불가, 재고없음:"+ this.count);
    	return null;
    	}
    
      // 고객이 낸 돈이 상품의 가격보다 부족한 경우
    if(price*count > money) {
    	System.out.println("판매불가, 돈 부족:"+price*count);
    	return null;
    }
    ...
    // 고객에게 반환할 Map 객체 (상품개수 + 잔돈)
    return Map.of("count", count, "change", price );
